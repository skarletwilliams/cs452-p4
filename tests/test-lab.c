...
/**
 * Test to check if the malloc behaves as expected when pool is null or size is 0!
 * 
 * If size is zero, the return value will be NULL
 * If pool is NULL, the return value will be NULL
 */
void test_buddy_malloc_edges(void) {
  fprintf(stderr, "->Test 0 and null values on malloc\n");
  struct buddy_pool pool;
  int kval = MIN_K;
  size_t size = UINT64_C(1) << kval;
  buddy_init(&pool, size);
...
/**
 * Tests the realloc funciton to ensure it behaves as expected. 
 * 
 * Content of memory block is preserved, up to the lesser of the sizes.
 * If the new size is larger, the value of the newly allocated portion is indeterminant.
 * If the ptr is null, the funciton acts as malloc.
 * If the size is 0 and the pointer is not null, then it acts as free.
 * 
 */
void test_buddy_realloc(void) {
  ...
  size_t size = UINT64_C(1) << kval;
  buddy_init(&pool, size);
  //This should work as malloc
  void *mem = buddy_realloc(&pool, NULL, 1);
  //Make sure correct kval was allocated
  struct avail *tmp = (struct avail *)mem - 1;
  assert(tmp->kval == SMALLEST_K);
  //assign a different number of bytes that still fits in OG k
  mem = buddy_realloc(&pool, mem, 2);
  tmp = (struct avail *)mem - 1;
  assert(tmp->kval == SMALLEST_K);
  // //assign a lot more
  // size_t ask = ((UINT64_C(1) << (MIN_K / 2))) - sizeof(struct avail);
  // mem = buddy_realloc(&pool, mem, ask);
  // assert(mem != NULL);


  //this should work as free
  buddy_realloc(&pool, mem, 0);
  check_buddy_pool_full(&pool);
  buddy_destroy(&pool);
}
...
int main(void) {
  ...
  RUN_TEST(test_buddy_malloc_edges);
